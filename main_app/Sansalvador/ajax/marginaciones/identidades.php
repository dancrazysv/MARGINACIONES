...
$TxtMargi1 = "Modifíquese el presente asiento de nacimiento, en el sentido que la persona inscrita es" . $sexoid . " " . trim(titleCase($conide)) . "; según Testimonio de Escritura Pública número " .$niden .", ". $dateid . ", ante " .trim($cargoid) ." " .trim(titleCase($nombreiden)) . ". Registro del Estado Familiar del Municipio de San Salvador Centro, San Salvador, " . $dia_actual . "" . $mes_actual . "" . $anio_actual.".";
...
$TxtMargi1 = "Modifíquese el presente asiento de defunción, en el sentido que la persona inscrita " . $sexoid2 . " " . trim(titleCase($conide2)) . "; según Testimonio de Escritura Pública número " .$niden2 .", ". $dateid . ", ante " .trim($cargoid) ." " .trim(titleCase($nombreiden)) . ". Registro del Estado Familiar del Municipio de San Salvador Centro, San Salvador, " . $dia_actual . "" . $mes_actual . "" . $anio_actual.".";



	$sql = "UPDATE margi SET TxtMargi1 = '" . Espacios($TxtMargi1) . "', AnioP = '$AnioP', LibroP = '" . Espacios($LibroP) . "', NPartida = '" . Espacios($NPartida) . "', InicialesPDF = '$Iniciales', FechaC = '$fechaActual2', TipoP = '$tipoparidedef', HoraC = '$hora', lineapdf = '$lin', registrador = '$jefer', nmargpdf = '$margpdfiden', cargor = '$carg', estado = '$est', TipoMargi= 'MIDEF', revestado = 'revisado1', lugar= '$tipousers' WHERE id = '$id' AND Iniciales1 = '$Iniciales'";
...
if ($tipoide == 'idenacdef' ) {
   ...
   $AnioP2 = mysqli_real_escape_string($con,(strip_tags($_POST['aniopart2ide'],ENT_QUOTES)));
	...
	// UPDATE data into database
$TxtMargi1 = trim(titleCase($niden)) . "" . $sexoid . " " .  trim(titleCase($conide)) . "; según Escritura Pública de Identidad" . $dateid . ", ante " . trim(titleCase($nombreiden)) . ", " . trim($cargoid) . " en " . trim($lugarid) . ". Registro del Estado Familiar del Municipio de San Salvador Centro, San Salvador, " . $dia_actual . "" . $mes_actual . "" . $anio_actual.".";
...
$TxtMargi2 = trim(titleCase($niden)) . "" . $sexoid2 . " " .  trim(titleCase($conide)) . "; según Escritura Pública de Identidad" . $dateid . ", ante " . trim(titleCase($nombreiden)) . ", " . trim($cargoid) . " en " . trim($lugarid) . ". Registro del Estado Familiar del Municipio de San Salvador Centro, San Salvador, " . $dia_actual . "" . $mes_actual . "" . $anio_actual.".";
		

$sql2 = "UPDATE margi SET TxtMargi1 = '" . Espacios($TxtMargi2) . "', AnioP = '$AnioP2', LibroP = '" . Espacios($LibroP2) . "', NPartida = '" . Espacios($NPartida2) . "', InicialesPDF = '$Iniciales', FechaC = '$fechaActual2', TipoP = '$tipoparidedef', HoraC = '$hora', lineapdf = '$lin', registrador = '$jefer', nmargpdf = '$margpdfiden2', cargor = '$carg', estado = '$est', TipoMargi= 'MIDEF', revestado = 'revisado1', lugar= '$tipousers' WHERE id = '$id2' AND Iniciales1 = '$Iniciales'";
