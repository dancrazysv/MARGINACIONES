...
function numtoletrasPDFM($xcifra)
{
    $xarray = array(0 => "CERO",
                    ...
                    case 1: // checa las centenas
                        if (substr($xaux, 0, 3) < 100) { // si el grupo de tres dígitos es menor a una centena ( < 99) no hace nada y pasa a revisar las decenas
                            ...
                            if (TRUE === array_key_exists($key, $xarray)){  // busco si la centena es número redondo (100, 200, 300, 400, etc..)
                                $xseek = $xarray[$key];
                                $xsub = subfijonum($xaux); // devuelve el subfijo2 correspondiente (Millón, Millones, Mil o nada)
...
function subfijonum($xx)
{ // esta función regresa un subfijo2 para la cifra
    $xx = trim($xx);
    $xstrlen = strlen($xx);
 ...
 * ----------------------------------------------------------------
 */
function fechaATextoM($fecha){
  $fecha_separada=explode("/", $fecha);

  $dia= numtoletrasPDFM($fecha_separada[0]);
  ...
  $anio= numtoletrasPDFM($fecha_separada[2]);
