$(function() {
    // Escucha el evento 'keyup' en el campo de búsqueda con un retardo (debounce).
    // Esto evita que se envíe una petición AJAX por cada tecla que el usuario presiona.
    // La función se ejecutará solo después de 300ms de inactividad.
    const searchInput = $("#q");
    searchInput.on('keyup', debounce(function() {
        load(1);
    }, 300));

    // Carga la página inicial al cargar el documento.
    load(1);
});

// Función de utilidad para "debouncing" (retardo de ejecución).
...
function load(page) {
    // Almacena selectores en variables para no tener que buscarlos en el DOM varias veces.
    const pageWrapper = $(".page-wrapper");
    const loader = $("#loader");
    const outerDiv = $(".outer_div");
    const query = $("#q").val();
    const perPage = 4;
    
    // Si la búsqueda es vacía, no se envía la petición. Esto puede ser útil para evitar
    // peticiones de carga innecesarias si el campo de búsqueda está vacío.
    // Puedes eliminar esta condición si quieres que siempre se carguen los datos.
    if (query === "" && page !== 1) {
        return;
    }

    // Oculta la tabla anterior de forma más eficiente.
    outerDiv.hide();
    
    // Muestra el loader con un mensaje claro.
    loader.html("<p>Cargando datos...</p>").show();
    ...
    $.ajax({
        ...
        data: parametros,
        method: 'POST', // Se recomienda usar POST para búsquedas para evitar problemas con la URL.
        success: function(data) {
            // Reemplaza el contenido y muestra la nueva tabla.
            outerDiv.html(data).fadeIn('slow');
            
            // Oculta el loader una vez que los datos han sido cargados.
            loader.fadeOut('slow', function() {
                // Restaura el contenido del loader si es necesario.
                $(this).empty();
            });
        },
        error: function(jqXHR, textStatus, errorThrown) {
            // Manejo de errores para retroalimentación al usuario.
            console.error("Error en la petición AJAX: " + textStatus, errorThrown);
            loader.html("<p style='color: red;'>Ocurrió un error al cargar los datos.</p>");
        }
    });
}
...
$("#autoprnac").hover(function(){
   if ($('.flat-red:checked').length == 0 & $('.ecuvar1:checked').length !== 0) {
    ...
    $(document).ready(function(){
        ...
        // Verificamos si los parámetros de apertura están presentes en la URL
        var shouldOpenModal = window.location.search.indexOf('open_modal=true') !== -1 || window.location.search.indexOf('buscar=true') !== -1;
        ...
        if (shouldOpenModal) {
            ...
            // 2. CORRECCIÓN CRÍTICA: LIMPIAR LA URL DEL HISTORIAL
            // Esto evita que el modal se abra en un F5 posterior.
            
            // Obtenemos el path base de la URL (ej: /index.php)
            var cleanUrl = window.location.pathname;
            
            // Reemplazamos el estado del historial para eliminar los parámetros de la URL
...
$(document).ready(function () {
        $("#tipo4").click(function () {
            if ($("#partida2").is(":visible")) {
             document.getElementById("matri1").style.display = 'none';
				...
				$("#Avalue").attr("checked", false);
					...
					document.getElementById("matriactas").style.display = 'none';
				$("#consuacta").prop("disabled", true);
				$("#nombresel").prop("disabled", true);
				$("#nombresella").prop("disabled", true);
				$("#muniel").prop("disabled", true);
				$("#muniella").prop("disabled", true);
				$("#actael").prop("disabled", true);
				$("#actaella").prop("disabled", true);
				$("#actael").attr("checked", false);
				$("#actaella").attr("checked", false);
			...
			document.getElementById("matriactas").style.display = '';
				$("#consuacta").prop("disabled", false);
				$("#nombresel").prop("disabled", true);
				$("#nombresella").prop("disabled", true);
				$("#muniel").prop("disabled", true);
				$("#muniella").prop("disabled", true);
				$("#actael").prop("disabled", false);
				$("#actaella").prop("disabled", false);
				$("#actael").attr("checked", false);
				$("#actaella").attr("checked", false);
				...
				document.getElementById("descritura").style.display = 'none';
               document.getElementById("dacta").style.display = 'none';

				$("#escrinumero").prop("disabled", true);
					$("#actanummat").prop("disabled", true);
					$("#libroacta").prop("disabled", true);
					$("#partidaacta").prop("disabled", true);

					$("#chknumpartida").prop("disabled", true);
				$("#librotipomatri").prop("disabled", true);
				$("chknumpartida").attr("checked", false);
...
$(document).ready(function () {
        $("#tipodoc1").click(function () {
            if ($("#descritura").is(":visible")) {
               document.getElementById("descritura").style.display = 'none';
               document.getElementById("dacta").style.display = 'none';
				...
				document.querySelectorAll('[id=tipodoc1]').forEach((x) => x.checked = false);

		  }
            else {
                 document.getElementById("descritura").style.display = '';
                 document.getElementById("dacta").style.display = 'none';
				...
				$("chknumpartida").attr("checked", false);
					...
					$("#escrinumero").prop("disabled", false);
					$("#actanummat").prop("disabled", true);
					$("#libroacta").prop("disabled", true);
					$("#partidaacta").prop("disabled", true);
        ...
        $("#tipodoc2").click(function () {
            if ($("#dacta").is(":visible")) {
               ...
               document.getElementById("dacta").style.display = 'none';
				...
				document.querySelectorAll('[id=tipodoc2]').forEach((x) => x.checked = false);

		  }
            else {
                 document.getElementById("descritura").style.display = 'none';
                 document.getElementById("dacta").style.display = '';
                 
				
					$("#escrinumero").prop("disabled", true);
					$("#actanummat").prop("disabled", false);
					$("#libroacta").prop("disabled", false);
					$("#partidaacta").prop("disabled", false);	


				$("#chknumpartida").prop("disabled", false);
				$("#librotipomatri").prop("disabled", true);
				$("chknumpartida").attr("checked", false);
        ...
        $("#tipodoc3").click(function () {
            if ($("#dacta").is(":visible")) {
               ...
               document.getElementById("dacta").style.display = 'none';
				...
				document.querySelectorAll('[id=tipodoc3]').forEach((x) => x.checked = false);
...
$(document).ready(function () {
        $("#tipo2d").click(function () {
            if ($("#nombres").is(":visible")) {
				...
				document.getElementById("nota3").style.display = 'none';
				document.getElementById("nota4").style.display = 'none';
				document.getElementById("tramitediv").style.display = 'none';
				$("#edit_el2").prop("disabled", true);
				$("#Nella").prop("disabled", true);
				$("#edit_anioe_div1").prop("disabled", true);
				$("#edit_libro_div1").prop("disabled", true);
				...
				document.getElementById("nota3").style.display = 'none';
				document.getElementById("nota4").style.display = 'none';
				document.getElementById("tramitediv").style.display = 'block';
						 $("#edit_el2").prop("disabled", false);
				$("#Nella").prop("disabled", false);
					$("#edit_anioe_div1").prop("disabled", false);
				$("#edit_libro_div1").prop("disabled", false);
...
$(document).ready(function () {
        $("#art17_4").click(function () {
          ...
          document.getElementById("art171").style.display = 'none';
				...
				document.querySelectorAll('[id=art17_4]').forEach((x) => x.checked = false);
            }
            else {
             document.getElementById("art171").style.display = '';
				 document.getElementById("art172").style.display = 'none';
...
$(document).ready(function () {
        $("#art17_5").click(function () {
          ...
          document.getElementById("art171").style.display = 'none';
				...
				document.querySelectorAll('[id=art17_5]').forEach((x) => x.checked = false);
            }
            else {
             document.getElementById("art171").style.display = '';
				 document.getElementById("art172").style.display = 'none';
...
$(document).ready(function () {
        $("#valuedivnjs").click(function () {
            if ($("#decreextnjs").is(":visible")) {
				...
				$("#decrevalnjs").attr("checked", false);
				$("#decrevalnjs").prop("disabled", true);
				$("#timedecrenjs").prop("disabled", true);
				$("#decreval2njs").attr("checked", false);
				$("#fechadecrenjs").prop("disabled", true);
				$("#por_sjuz").attr("checked", false);
...
$(document).ready(function () {
        $("#valuedivofi").click(function () {
            if ($("#decreextofi").is(":visible")) {
				...
				$("#decrevalofi").attr("checked", false);
				$("#decrevalofi").prop("disabled", true);
				$("#timedecreofi").prop("disabled", true);
				$("#decreval2ofi").attr("checked", false);
				$("#fechadecreofi").prop("disabled", true);
				$("#por_rofi").attr("checked", false);
...
$(document).ready(function () {
        $("#valuedivnjse").click(function () {
            if ($("#decreextnjse").is(":visible")) {
				...
				$("#decrevalnjse").attr("checked", false);
				$("#decrevalnjse").prop("disabled", true);
				$("#timedecrenjse").prop("disabled", true);
				$("#decreval2njse").attr("checked", false);
				$("#fechadecrenjse").prop("disabled", true);
				$("#por_ejuz").attr("checked", false);
...
function showContent() {
        element = document.getElementById("foliacionmatr");
        check = document.getElementById("actael");
        if (check.checked) {
...
function showContentdos() {
        element = document.getElementById("foliacionmatr2");
        check = document.getElementById("actaella");
        if (check.checked) {
            element.style.display='';
...
///////////////// se realiza la consulta de actas en la base de datos de matrimonios alcalde
$(function() {
            $("#consuacta").autocomplete({
                source: "consuactas.php",
                minLength: 2,
                select: function(event, ui) {
					event.preventDefault();
					$('#consuacta').val(ui.item.actanum);
					$('#nombresel').val(ui.item.nombresel);
					$('#nombresella').val(ui.item.nombresella);
					$('#muniel').val(ui.item.origenel);
					$('#muniella').val(ui.item.origenella);
					$('#actanombresel').val(ui.item.actanombreel);
					$('#actanombresella').val(ui.item.actanombreella);
					$('#actaapeunoel').val(ui.item.apeunoel);
					$('#actaapedosel').val(ui.item.apedosel);
					$('#actaapeunoella').val(ui.item.apeunoella);
					$('#actaapedosella').val(ui.item.apedosella);
					$('#actaalcalde').val(ui.item.alcalde);
					$('#actaregimen').val(ui.item.regimen);
					$('#actausoapellidos').val(ui.item.usoapellidos);
					$('#actafechamatri').val(ui.item.fechamatrimonio);
					$('#actaconoel').val(ui.item.conoel);
					$('#actaconoella').val(ui.item.conoella);
...
///////////////// se realiza la consulta de actas en la base de datos de ingreso desde recepcion
$(function() {
            $("#numtramite").autocomplete({
                source: "consutramite.php",
                minLength: 2,
                select: function(event, ui) {
					event.preventDefault();
					$('#apellidoel1').val(ui.item.ape1elt);
...
$(function() {
            $("#numtramite").autocomplete({
                ...
                select: function(event, ui) {
					event.preventDefault();
					$('#numtramite').val(ui.item.numsolit);

					$('#edit_el').val(ui.item.nombreselt);
					$('#apellidoel1').val(ui.item.ape1elt);

					$('#Nellamat').val(ui.item.nombresellat);
					$('#apellidoella1').val(ui.item.ape1ellat);
				
					
					

					const chkape2el = ui.item.chkape2el;
          if (chkape2el == "apellidoel") {
      document.getElementById("Rvalue2").checked = true;
      document.getElementById('apellidoelid').disabled = false;
      $('#apellidoelid').val(ui.item.ape2elt);

    } else {
	document.getElementById("Rvalue2").checked = false;
	document.getElementById('apellidoelid').disabled = true;
      $('#apellidoelid').val('');

    } 


const chkape2ella = ui.item.chkape2ella;
          if (chkape2ella == "apellidoella") {
      document.getElementById("Rvalue3").checked = true;
     document.getElementById('apellidoellaid').disabled = false;
      $('#apellidoellaid').val(ui.item.ape2ellat);

    } else {
	document.getElementById("Rvalue3").checked = false;
	 document.getElementById('apellidoellaid').disabled = true;
      $('#apellidoellaid').val('');
    } 


		const chkconoel = ui.item.chkconoel;
          if (chkconoel == "conocidoel") {
      document.getElementById("conocido1").checked = true;
      document.getElementById('conotxt').disabled = false;
      $('#conotxt').val(ui.item.conocidoelt);

    } else {
	document.getElementById("conocido1").checked = false;
	document.getElementById('conotxt').disabled = true;
      $('#conotxt').val('');

    } 


const chkconoella = ui.item.chkconoella;
          if (chkconoella == "conocidoella") {
      document.getElementById("conocido2").checked = true;
      document.getElementById('conotxt2').disabled = false;
      $('#conotxt2').val(ui.item.conocidoellat);

    } else {
	document.getElementById("conocido2").checked = false;
	document.getElementById('conotxt2').disabled = true;
      $('#conotxt2').val('');

    } 
					
				
$('#FechaB').val(ui.item.fechamatrit);
$('#LugarB').val(ui.item.lugarmatrit);



const chkcfuncionario = ui.item.chkcfuncionario;
          if (chkcfuncionario == "chkcfuncionario") {
      document.getElementById("ccargo").checked = true;
      document.getElementById('options').disabled = false;
      $('#options').val(ui.item.cfuncionariot);

    } else {
	document.getElementById("ccargo").checked = false;
...
const chknfuncionario = ui.item.chknfuncionario;
          if (chknfuncionario == "chknfuncionario") {
      document.getElementById("mnombre").checked = true;
      document.getElementById('Nnotario').disabled = false;
      $('#Nnotario').val(ui.item.nfuncionariot);

    } else {
	document.getElementById("mnombre").checked = false;
...
const chkregimen = ui.item.chkregimen;
          if (chkregimen == "chkregimen") {
      document.getElementById("OptionRA").checked = false;
      document.getElementById('treg').disabled = true;
      $('#treg').val(ui.item.regiment);
...
const chkotroape = ui.item.chkotroape;
          if (chkotroape == "chkotroape") {
document.getElementById("Avalue").checked = true;
const usoapet = ui.item.usoapet;
          	if (usoapet == "conde") {
document.getElementById('usoA').disabled = false;
document.getElementById('otroapellido').disabled = true;
$('#otroapellido').val('');
$('#usoA').val(ui.item.usoapet);
 } else if (usoapet == "sinde") {
document.getElementById('usoA').disabled = false;
document.getElementById('otroapellido').disabled = true;
$('#otroapellido').val('');
$('#usoA').val(ui.item.usoapet);
} else if (usoapet == "soltera") {
document.getElementById('usoA').disabled = false;
document.getElementById('otroapellido').disabled = true;
$('#otroapellido').val('');
$('#usoA').val(ui.item.usoapet);
} else if (usoapet == "otro") {
document.getElementById('usoA').disabled = false;
document.getElementById('otroapellido').disabled = false;
$('#otroapellido').val(ui.item.otroapellido);
$('#usoA').val(ui.item.usoapet);
}
	...
	document.getElementById("Avalue").checked = false;
	document.getElementById('usoA').disabled = true;
	document.getElementById('otroapellido').disabled = true;
      $('#otroapellido').val('');
...
$(function() {
            $("#numtramitediv").autocomplete({
                source: "consutramitediv.php",
                minLength: 2,
                select: function(event, ui) {
					event.preventDefault();
					$('#numtramitediv').val(ui.item.numsolitdiv);

			
					
					const chksendivs = ui.item.chksendiv;
          if (chksendivs == "chksentencia") {
      document.getElementById("valuedivnjs").checked = true;
      document.getElementById('decreextnjs').style.display = "block";
     document.getElementById('decrevalnjs').disabled = true;

    } else {
	document.getElementById("valuedivnjs").checked = false;
      document.getElementById('decreextnjs').style.display = "none";
document.getElementById('decrevalnjs').disabled = true;
    } 


const chkfechasdiv = ui.item.chksfechadiv;
          if (chkfechasdiv == "chksfecha") {
      document.getElementById('decreval2njs').disabled = false;
      document.getElementById("decreval2njs").checked = true;
      document.getElementById('fechadecrenjs').disabled = false;
      $('#fechadecrenjs').val(ui.item.fechasendiv);
     

    } else {
    document.getElementById('decreval2njs').disabled = true;	
	document.getElementById("decreval2njs").checked = false;
	document.getElementById('fechadecrenjs').disabled = true;
    $('#fechadecrenjs').val('');

    } 

const lsentencia = ui.item.lsendiv+" de "+ui.item.lsentencia;

const chkporsdiv = ui.item.chkspordiv;
          if (chkporsdiv == "chkspore") {
      document.getElementById('por_sjuz').disabled = false;
      document.getElementById("por_sjuz").checked = true;
      document.getElementById('txt_por_sjuz').disabled = false;
      $('#txt_por_sjuz').val(lsentencia);
     

    } else {
    document.getElementById('por_sjuz').disabled = true;	
	document.getElementById("por_sjuz").checked = false;
	document.getElementById('txt_por_sjuz').disabled = true;
    $('#txt_por_sjuz').val('');
...
const chkoficiodiv = ui.item.chkoficiodiv;
if (chkoficiodiv == "chkoficio") {
      document.getElementById("valuedivofi").checked = true;
      document.getElementById('decreextofi').style.display = "block";
     document.getElementById('decrevalofi').disabled = true;

    } else {
	document.getElementById("valuedivofi").checked = false;
      document.getElementById('decreextofi').style.display = "none";
document.getElementById('decrevalofi').disabled = true;
    } 
...
const chkfechaodiv = ui.item.chkofechadiv;
          if (chkfechaodiv == "chkofecha") {
      document.getElementById('decreval2ofi').disabled = false;
      document.getElementById("decreval2ofi").checked = true;
      document.getElementById('fechadecreofi').disabled = false;
      $('#fechadecreofi').val(ui.item.fechaoficiodiv);
     

    } else {
    document.getElementById('decreval2ofi').disabled = true;	
	document.getElementById("decreval2ofi").checked = false;
	document.getElementById('fechadecreofi').disabled = true;
    $('#fechadecreofi').val('');
...
const chkooficio = ui.item.chkooficiodiv;
          if (chkooficio == "chkooficio") {
      document.getElementById('por_ofi').disabled = false;
      document.getElementById("por_ofi").checked = true;
      document.getElementById('txt_por_ofi').disabled = false;
      $('#txt_por_ofi').val(ui.item.numoficiodiv);
     

    } else {
    document.getElementById('por_ofi').disabled = true;	
	document.getElementById("por_ofi").checked = false;
	document.getElementById('txt_por_ofi').disabled = true;
    $('#txt_por_ofi').val('');

    } 
const loficios = ui.item.insoficiodiv+" de "+ui.item.loficios;

const chkopordiv = ui.item.chkopordiv;
          if (chkopordiv == "chkopor") {
      document.getElementById('por_rofi').disabled = false;
      document.getElementById("por_rofi").checked = true;
      document.getElementById('txt_por_rofi').disabled = false;
      $('#txt_por_rofi').val(loficios);
     

    } else {
    document.getElementById('por_rofi').disabled = true;	
	document.getElementById("por_rofi").checked = false;
	document.getElementById('txt_por_rofi').disabled = true;
    $('#txt_por_rofi').val('');
...
$('#otras').on('shown.bs.modal', function () {
    $('#tipomargvacia').select2({
        placeholder: "Escriba para buscar...",
        width: '100%',
        dropdownParent: $('#otras')
    });
});
...
const chkejedivs = ui.item.chkejediv;
          if (chkejedivs == "chkejecutoria") {
      document.getElementById("valuedivnjse").checked = true;
      document.getElementById('decreextnjse').style.display = "block";
      document.getElementById('decrevalnjse').disabled = true;
     

    } else {
	document.getElementById("valuedivnjse").checked = false;
      document.getElementById('decreextnjse').style.display = "none";
document.getElementById('decrevalnjse').disabled = true;
    } 


const chkfechaediv = ui.item.chkefechadiv;
          if (chkfechaediv == "chkefecha") {
      document.getElementById('decreval2njse').disabled = false;
      document.getElementById("decreval2njse").checked = true;
      document.getElementById('fechadecrenjse').disabled = false;
      $('#fechadecrenjse').val(ui.item.fechaejediv);
     

    } else {
    document.getElementById('decreval2njse').disabled = true;	
	document.getElementById("decreval2njse").checked = false;
	document.getElementById('fechadecrenjse').disabled = true;
    $('#fechadecrenjse').val('');
...
const lejecutoria = ui.item.lejediv+" de "+ui.item.lejecutoria;


const chkporediv = ui.item.chkepordiv;
          if (chkporediv == "chkepor") {
      document.getElementById('por_ejuz').disabled = false;
      document.getElementById("por_ejuz").checked = true;
      document.getElementById('txt_por_ejuz').disabled = false;
      $('#txt_por_ejuz').val(lejecutoria);
     

    } else {
    document.getElementById('por_ejuz').disabled = true;	
	document.getElementById("por_ejuz").checked = false;
	document.getElementById('txt_por_ejuz').disabled = true;
    $('#txt_por_ejuz').val('');
...
const chknombresdiv = ui.item.chknombresdiv;
const chkconoeldiv = ui.item.chkconoeldiv;
const chkconoelladiv = ui.item.chkconoelladiv;


const resultadoel = ui.item.nombreseldiv+" conocido por "+ui.item.conocidoeldiv;
const resultadoella = ui.item.nombreselladiv+" conocida por "+ui.item.conocidoelladiv;



          if (chknombresdiv == "chknombres") {
     if (chkconoeldiv == "conocidoel") {
      document.getElementById('edit_el2').disabled = false;
     	$('#edit_el2').val(resultadoel);
     } else {
	$('#edit_el2').val(ui.item.nombreseldiv);
} 

    if (chkconoelladiv == "conocidoella") {
       document.getElementById('Nella').disabled = false;
	     $('#Nella').val(resultadoella);
} else {
	$('#Nella').val(ui.item.nombreselladiv);
} 


    } else {
    document.getElementById('edit_el2').disabled = true;
      document.getElementById('Nella').disabled = true;
    $('#edit_el2').val('');
    $('#Nella').val('');
