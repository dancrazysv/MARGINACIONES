...
$('#validarcmargi').on('show.bs.modal', function (event) {
		  var button = $(event.relatedTarget) // Button that triggered the modal
		  var idrevi = button.data('idrevision') 
		  $('#validarid').val(idrevi)
			 		  

		})

$('#validarccontrol').on('show.bs.modal', function (event) {
		  var button = $(event.relatedTarget) // Button that triggered the modal
		  var idrevic = button.data('idrevisionc') 
		  $('#validaridc').val(idrevic)

		  var idrevtres = button.data('textrevision') 
		  $('#validariinidc').val(idrevtres)

		  var idrevcuatro = button.data('nlibrev') 
		  $('#validaredidos').val(idrevcuatro)

		   var idrevcinco = button.data('txtlugar') 
		  $('#validarlugar').val(idrevcinco)
...
$( '#formvalidarccontrol' ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
					type: "POST",
					url: "ajax/editar_producto.php",
...
function myFunction20() {
  document.getElementById("formvalidarcmargi").reset();
		  
	
	}

	function myFunction21() {
  document.getElementById("formvalidarccontrol").reset();
